...
async def init_users_database() -> bool:  # Init database
    async with aiosqlite.connect(database_path) as db:
        try:
            await db.execute(init_db_users_sql_script)  # Execute script
...
async def create_user(userdata: list) -> bool:  # Create user func
    async with aiosqlite.connect(database_path) as db:
        try:
